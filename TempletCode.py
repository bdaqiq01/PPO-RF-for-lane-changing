...
#TODO loss function for PPO
def PPoLossFromLogits(
    logits,                 # policy network raw outputs, You convert logits → action probabilities using a softmax, and then either sample or pick the max. N number of rows and A are the number of action
    actions,                # taken actions (N,)
    old_log_probs,          # log π_old(a_t|s_t) recorded when the data was collected (N,)
    advantages,             # Â_t (N,)
    values,                 # V_θ(s_t) current value predictions (N,)
    value_targets,          # V_targ,t (e.g., GAE targets) (N,)
    clip_eps=0.2,           # ε in Eq. (3)
    c1=0.5,                 # value loss weight in Eq. (2)
    c2=0.01                 # entropy bonus weight in Eq. (2)
):
    """
    Returns:
        loss         : scalar to minimize  = policy_loss + c1*value_loss - c2*entropy
        stats (dict) : handy scalars for logging
    """

    # π_θ(a_t|s_t) and entropy
    dist = Categorical(logits=logits)
    new_log_probs = dist.log_prob(actions)                # log π_θ(a_t|s_t)
    entropy = dist.entropy().mean()                       # S[π_θ](s_t)  (Eq. 2)

    # Probability ratio r_t(θ) = π_θ / π_old  (Eq. 3)
    ratio = torch.exp(new_log_probs - old_log_probs)

    # Clipped surrogate objective L_CLIP (Eq. 3)
    surr1 = ratio * advantages
    surr2 = torch.clamp(ratio, 1.0 - clip_eps, 1.0 + clip_eps) * advantages
    policy_obj = torch.min(surr1, surr2).mean()           # E_t[ min(…) ]

    # Value function loss L_VF = (V_θ(s_t) - V_targ,t)^2 (Eq. 2)
    value_loss = F.mse_loss(values, value_targets)

    # Full PPO objective L_CLIP+VF+S (Eq. 2)
    objective = policy_obj - c1 * value_loss + c2 * entropy

    # We minimize negative objective (the “loss” used by the optimizer)
    loss = -objective

    stats = {
        "policy_obj": policy_obj.item(),
...
#it is inthe reward_functions.py now


#implemeted in the utils state extraction.py
...
# def apply_action(action, tls_id="Node2"): #5. Constraint 5 supposed to be done by the step function in the environment 
#     """
